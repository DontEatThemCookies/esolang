# Super Stack! by User:Orange in the Esolang wiki
# Ported to Python 3 by David Costell - davidcostell44@gmail.com
# 10/30/21
...
The [-d] parameter is optional, it is used to print out the stack.
"""
...
def run(text, debug=0):
    ...
    while pc < len(prog):
        ...
        if inst == 'add':
            ...
            stack.append(random.randrange(stack.pop()))
        ...
        elif inst == 'outputascii':
            sys.stdout.write(chr(stack.pop()))
        elif inst == 'inputascii':
            inpascii = list((map(ord, input('')))) # Map object was typed list for Python3 compatibility (David)
            stack.extend(inpascii[::-1]) # Ah, slice steps
    ...
    if stack and debug:
        print('\nprogram ended with not empty stack:\n', stack)


def usage():
    print('Usage: superstack.py [program name].ss! [-d]')
    print('    -d  debug')
    sys.exit(2)
